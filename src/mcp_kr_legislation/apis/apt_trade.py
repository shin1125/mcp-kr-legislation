...
def get_apt_trade_data(lawd_cd: str, deal_ymd: str) -> str:
    """
    아파트 매매 실거래가 API 호출 및 전체 데이터 JSON+통계 반환 (curl subprocess + requests fallback, 반복 수집)
    Args:
    ...
    file_path = data_dir / f"APT_{lawd_cd}_{deal_ymd}.xml"
